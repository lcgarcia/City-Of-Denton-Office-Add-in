...
var BudgetSQLGenerator = function(options) {
  this.schema = "proddta";
  if (options && options.schema) this.schema = options.schema;
...
BudgetSQLGenerator.prototype.getBusinessUnitData = function(reportSelected) {
  this.type = reportSelected || this.type;
  let sql =
    "SELECT a.mcco,b.ccname,a.mcmcu,a.mcdl01 " +
...
BudgetSQLGenerator.prototype.createSelectStatement = function(
  adHoc,
  reportSelected,
  forYear,
  forMonth,
 ...
 */
BudgetSQLGenerator.prototype.select = function(
  reportSelected,
  forMonth,
  forYear
) {
  if (forMonth !== "13th") forMonth = forMonth.substr(0, 3);
  ...
  var YTDBudget =
    forMonth != "Jun" ? `${YTD}+a.GBBORG` : `${YTD}+a.GBAPYC+a.GBBORG`;
  
  var annualPNL =
    "a.GBAN01+a.GBAN02+a.GBAN03+a.GBAN04+a.GBAN05+a.GBAN06+a.GBAN07+a.GBAN08+a.GBAN09+a.GBAN10+a.GBAN11+a.GBAN12+a.GBAN13";
  var annualBS =
    "a.GBAPYC+ a.GBAN01+a.GBAN02+a.GBAN03+a.GBAN04+a.GBAN05+a.GBAN06+a.GBAN07+a.GBAN08+a.GBAN09+a.GBAN10+a.GBAN11+a.GBAN12+a.GBAN13";
  var originalBudget = "a.GBBORG";
  
  var income =
    reportSelected.toLowerCase() == "between '4000' and '9999'" ? true : false;
  ...
  switch (this.type) {
    case "f":
      ...
      select += `(CASE WHEN trim(a.GBSBL) = '' Or trim(a.gbsbl) is null THEN 'BLANK SUBLEDGER' ELSE TO_CHAR(a.GBSBL) END) as GBSBL, `;
  ...
  select += `SUM(CASE WHEN a.GBLT = 'AA' and a.GBFY = ${forYear - 1} THEN ${
    income ? annualPNL : annualBS
  } ELSE 0 END)/100 as PriorAnnualAct, `;
  select += `SUM(CASE WHEN a.GBLT = 'AA' and a.GBFY = ${forYear - 1} THEN ${
    income ? YTDPNL : YTD
  } ELSE 0 END)/100 as PriorYTDAct, `;
  select += `SUM(CASE WHEN a.GBLT = 'BA' and a.GBFY = ${forYear} THEN ${
    income ? originalBudget : annualBS
  } ELSE 0 END)/100 as CurrentOrigBud, `;
  select += `SUM(CASE WHEN a.GBLT = 'BA' and a.GBFY = ${forYear} THEN ${
    income ? annualPNL : originalBudget
  } ELSE 0 END)/100 as CurrentModBud, `;
  select += `SUM(CASE WHEN a.GBLT = 'BA' and a.GBFY = ${forYear} THEN ${
    income ? annualPNL : annualBS
  } ELSE 0 END)/100 - `;
  select += `SUM(CASE WHEN a.GBLT = 'PA' and a.GBFY = ${forYear} THEN ${YTD} ELSE 0 END)/100 - `;
  select += `SUM(CASE WHEN a.GBLT = 'AA' and a.GBFY = ${forYear} THEN ${
    income ? YTDPNL : YTD
  } ELSE 0 END)/100 as CurrentAvailBal, `;
  select += `SUM(CASE WHEN a.GBLT = 'BA' and a.GBFY = ${forYear} THEN ${
    income ? YTDPNL : YTD
  } ELSE 0 END)/100 as CurrentYTDBud, `;
  select += `SUM(CASE WHEN a.GBLT = 'PA' and a.GBFY = ${forYear} THEN ${YTD} ELSE 0 END)/100 as CurrentYTDEnc, `;
  select += `SUM(CASE WHEN a.GBLT = 'AA' and a.GBFY = ${forYear} THEN ${
    income ? YTDPNL : YTD
  } ELSE 0 END)/100 as CurrentYTDAct, `;
  select += `SUM(CASE WHEN a.GBLT = 'PA' and a.GBFY = ${forYear} THEN ${YTD} ELSE 0 END)/100 +`;
  select += `SUM(CASE WHEN a.GBLT = 'AA' and a.GBFY = ${forYear} THEN ${
    income ? YTDPNL : YTD
  } ELSE 0 END)/100 as CurrentYTDTotal, `;
  ...
  select += `SUM(CASE WHEN a.GBLT = 'BA' and a.GBFY = ${forYear + 1} THEN ${
    income ? originalBudget : annualBS
  } ELSE 0 END)/100 as ProposedAnnualBud `;
...
BudgetSQLGenerator.prototype.whereClause = function(data) {
  ...
  switch (this.type) {
    ...
    case "e":
    case "a":
      sql = `WHERE ( (b.GMAID = a.GBAID(+)) AND ((b.GMOBJ ${
        data.accounts
      })) AND ((${where}))`;
  ...
  sql += ` AND ((a.GBFY in (${year - 1},${year},${year +
    1}) OR a.GBFY IS NULL )))`;
 ...
 */
BudgetSQLGenerator.prototype.adHocWhere = function(
  companyKey,
  businessUnitKey
) {
  var prefix = "b.GM";
  ...
  businessUnitKey = businessUnitKey.trim();
  
  var company =
    companyKey && companyKey != "" ? `${prefix}CO in (${companyKey})` : "";
  var businessUnit =
    businessUnitKey && businessUnitKey != "" ?
    `${prefix}MCU in (${businessUnitKey})` :
    "";
...
BudgetSQLGenerator.prototype.from = function() {
  switch (this.type) {
    case "f":
      return `FROM ${this.schema}.F0902 a,${this.schema}.F0101 b,${
        this.schema
      }.F0901 c `;
      break;
    default:
...
BudgetSQLGenerator.prototype.groupBy = function() {
  switch (this.type) {
    case "f":
      ...
      group += `END),`;
      group += `(CASE WHEN trim(a.GBSBL) = '' Or trim(a.gbsbl) is null THEN 'BLANK SUBLEDGER' ELSE TO_CHAR(a.GBSBL) END), `;
