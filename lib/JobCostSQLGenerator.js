...
JobCostSQLGenerator.prototype.getJobString = function(job, status) {
  ...
  if(status) {
    if(status.toUpperCase() === '*ALL') return `${jobStr} and b.MCPECC <> '*ALL' `
    else if (status.toUpperCase() === 'OPEN') return `${jobStr} and b.MCPECC in (' ','K') `
    else if(status.toUpperCase() === 'CLOSED') return `${jobStr} and b.MCPECC = 'N' `
    else return `${jobStr} and b.MCPECC <> '*ALL' `
  } 
  else return jobStr
};
...
JobCostSQLGenerator.prototype.orderBy = function(reportSelected, layout) {
  if(reportSelected == 'e') {
    ...
    else if (layout == 'Cost Code/Type Details') return "ORDER BY 3,5,6 "
    else return "ORDER BY 3,4,1,2 "
  } 
  else {
