...
JobCostSQLGenerator.prototype.getCadeCodeDetail = function(department, company, project, status, job, catCode, reportSelected) {
  ...
  return `SELECT DISTINCT ${catCodeSelect} as key, ${catCodeSelect} || '   ' || b.drdl01 as name `
...
JobCostSQLGenerator.prototype.getJobString = function(job, status) {
  ...
  if(status) {
    if(status.toLowerCase() === 'closed') return `${jobStr} and b.MCPECC = 'N' `
    else if (status.toLowerCase() === 'open') return `${jobStr} and b.MCPECC in (' ','K') `
  } else return jobStr
};
...
JobCostSQLGenerator.prototype.getDepartmentString = function(department) {
  if (department.toUpperCase() === '*ALL') return "b.MCRP27 <> '60' "
...
JobCostSQLGenerator.prototype.groupBy = function(reportSelected, layout) {
  if(reportSelected == 'e') {
    ...
    if (layout.toLowerCase() == 'no details') return "GROUP BY b.MCRP27,a.GBCO,b.MCMCUS,a.GBMCU || ' ' || b.MCDL01 "
    ...
    if (layout.toLowerCase() == 'no details') return "GROUP BY b.MCRP27,a.GBCO,b.MCMCUS,a.GBMCU || ' ' || b.MCDL01 "
    else if (layout.toLowerCase() == 'cost code/type details') return "GROUP BY b.MCRP27,a.GBCO,b.MCMCUS,a.GBMCU || ' ' || b.MCDL01,a.GBOBJ,a.GBSUB "
    else return ''
  }
};
...
JobCostSQLGenerator.prototype.orderBy = function(reportSelected, layout) {
  if(reportSelected == 'e') {
    if (layout.toLowerCase() == 'ferc details') return "ORDER BY 3,6,5 "
    else return "ORDER BY 3,4,1,2 "
  } else if (reportSelected == 'ka') {
    if(layout.toLowerCase() == 'ferc/cost code subtotals') return "ORDER BY 3,6,5 "
    else if (layout.toLowerCase() == 'cost type subtotals') return "ORDER BY 3,5,6 "
    else return "ORDER BY 3,4,1,2 "
  } else {
