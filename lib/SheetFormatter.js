...
module.exports = {
  mapResultsToArray: (results, showBlank) => _.map(results, val => {
    ...
    if(val.MCMCUS && val.MCMCUS.trim() == "") val.MCMCUS = "Not Assigned in JDE";
    return _.values(val)
  }),
  ...
  mapManagerResultsToArray: (results, managers) => _.map(results, (val) => {
    ...
    var object = rows[4] ? rows[4].trim() : null;
    ...
    _.map(groupedByKey, (val, key) => {
      if(!removeSubtotals){
        ...
        if (optionalProjectList) {
          ...
          var projectName = (val[0][2] && val[0][2].trim() != "") ? val[0][2] : val[0][3];
          projectName = project ? project.name : projectName;
          row.push(projectName.trim() + " Total");
          _.times(2, () => row.push(''));
        }
        else {
          row.push(val[0][2]);
  ...
  mapHiddenRows: (subTotalRows, headerOffset, optionalProjects, managers) =>
    _.map(subTotalRows, (val) => {
      const range = `A${headerOffset+1}:Z${val.length+(headerOffset-1)}`;
      ...
      if (optionalProjects) {
        ...
        var projectName = (!managers && val[0][2] && val[0][2].trim() != "") ? val[0][2] : val[0][3];
        key = project ? project.name : projectName;
      }
