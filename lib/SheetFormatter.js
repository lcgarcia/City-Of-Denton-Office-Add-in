...
module.exports = {
  ...
  addSubTotalRows: (groupedByKey, columnNames, headerOffset, rowPadding, optionalProjectList, layout) => 
    _.map(groupedByKey, (val, key) => {
      let fercRow = ['','','','','',val[0][5]+' Total'];
      let row = _.clone(rowPadding);
      if(layout == "Cost Type Subtotals") fercRow = ['','','','',val[0][4]+' Total',''];
      ...
      columnNames.forEach(letter => {
        ...
        fercRow.push(`=SUBTOTAL(9,${letter}${headerOffset+1}:${letter}${headerOffset+val.length})`);
      });
      
      if(layout == "FERC Details" || layout == "FERC/Cost Code Subtotals" || layout == "Cost Type Subtotals") val.push(fercRow);
      val.push(row);
  ...
  addManagerSubTotalRows: (groupedByKey, columnNames, headerOffset, rowPadding, layout) => 
    _.map(groupedByKey, (val, key) => {
      let fercRow = ['','','','','','',val[0][6]+' Total'];
      let row = _.clone(rowPadding);
      if(layout == "Cost Type Subtotals") fercRow = ['','','','','',val[0][5]+' Total','']; 
      ...
      if(layout == "FERC/Cost Code Subtotals" || layout == "Cost Type Subtotals") val.push(fercRow);
      val.push(row);
      headerOffset += val.length;
      return val;
    }),
    ...
    _.map(subTotalRows, (val) => {
      ...
      headerOffset += val.length;
      return { key, range, rows, subTotalRow };
    }),
