...
const jobSheetDataQuery = (sql, projects, managers, layout) => {
  ...
  .then(result => {
    ...
    if(managers){
      ...
      if(values && values.length > 0) projectIndex = values[0].length-1;
    }
    _.forEach(values, (val) => {
    ...
    // Get subTotalRows
    const subTotalRows = SheetFormater.mapSubTotalRows(groupedByKey, offSet+1, layout == "FERC Details" || layout == "FERC/Cost Code Subtotals");

    // Recombine with subtotals
    if(managers){
      subTotaled = SheetFormater.addManagerSubTotalRows(groupedByKey, columnNames, offSet, rowPadding, layout == "FERC/Cost Code Subtotals");
    }
    else subTotaled = SheetFormater.addSubTotalRows(groupedByKey, columnNames, offSet, rowPadding, projects, layout == "FERC Details");
