...
const budgetSheetFDataQuery = (sql) => {
  ...
  .then(result => {
    ...
    return { sheetData, hiddenRows, subTotalRows, allSubTotalRows };
  });
}
...
const jobSheetDataQuery = (sql, projects, managers, options) => {
  ...
  .then(result => {
    ...
    if(managers){
      ...
      if(options.trend) incomeValues = SheetFormater.mapTrendResultsToArray(incomeValues, managers);
      else incomeValues = SheetFormater.mapManagerResultsToArray(incomeValues, managers);
      if(incomeValues && incomeValues.length > 0) projectIndex = incomeValues[0].length-1;
    }
    ...
    // Get subTotalRows
    const subTotalRows = SheetFormater.mapSubTotalRows(groupedByKey, offSet+1, layout);

    // Recombine with subtotals
    if(managers){
      if(options.trend){
        var header = options.trend.report == 'ka' ? ['Dept','Company','Project Manager','Project','Bus Unit','Object'] : ['Dept','Company','Project','Bus Unit','Object','Subsidiary'];
        var grandTotal = [];
        options.trend.lastColumn = 'F';
        if(subTotalRows.length > 0){
          subTotaled = SheetFormater.addTrendSubTotalRows(groupedByKey, offSet, rowPadding, options.trend.periods);
          ...
          if(columnLength > 26){
            ...
            options.trend.lastColumn = alphaMap[leftLetter]+""+alphaMap[rightLetter];
          }
          else options.trend.lastColumn = alphaMap[columnLength];
          
          const monthValues = ['October','November','December','January','February','March','April','May','June','July','August','September'];
          ...
          for (let i=1; i<=options.trend.periods; i++) {
            ...
            grandTotal.push(`=SUBTOTAL(9,${alphaMap[alphaKey1]}${alphaMap[alphaKey2]}${offSet+1}:${alphaMap[alphaKey1]}${alphaMap[alphaKey2]}${lastRow})`);
      ...
      else subTotaled = SheetFormater.addManagerSubTotalRows(groupedByKey, columnNames, offSet, rowPadding, layout);
    }
    else subTotaled = SheetFormater.addSubTotalRows(groupedByKey, columnNames, offSet, rowPadding, projects, layout);

    // Get rows to hide
    let headerOffset = 6;
    if(options.trend) hiddenRows = SheetFormater.mapTrendHiddenRows(subTotaled, headerOffset, options.trend.lastColumn);
    else hiddenRows = SheetFormater.mapHiddenRows(subTotaled, headerOffset, projects);
    ...
    var jsonHiddenData = JSON.stringify(hiddenRows);
    if(hiddenRows.length > 200) jsonHiddenData = JSON.stringify(hiddenRows.slice(0, 200));
    jsonHiddenData = jsonHiddenData.replace(/\"\"/g, '"');
    jsonHiddenData = jsonHiddenData.replace(/\\"/g, "\"");
    jsonHiddenData = jsonHiddenData.replace(/\'/g, '');

    return { sheetData, hiddenRows, jsonHiddenData, subTotalRows, sql, trend };
  });
}
