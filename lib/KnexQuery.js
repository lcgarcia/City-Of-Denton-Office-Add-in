  ...
  .then(result => {
    ...
    const addSubTotal = obj => {
      if (obj.val) {
        ...
        if (newObj[3] == 4) {
          hiddenRows.push({ range: `A${obj.index+1+headerOffset}:Z${sheetData.length+headerOffset-1}`, name: newObj[4] });
...
module.exports = { query, jobSheetDataQuery, budgetSheetDataQuery };