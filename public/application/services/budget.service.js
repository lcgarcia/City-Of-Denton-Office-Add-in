  ...
  function($http) {
    ...
    this.getSheetData = function(type, keys, month, buLevel, year, accounts, options) {
      var makeRequest = function(cb) {
        ...
        return $http.post('/ks2inc/budget/sheet/data', JSON.stringify(data), {
            headers: {
    ...
    var deleteAllSheets = function(data, next) {
      Excel.run(function(ctx) {
        var ids = _.map(data.items, function(sheet) {
          return sheet.id
        });
      ...
      Excel.run(function(ctx) {
        ...
        var ranges = [{
          ...
          },
          {
            ...
            subTitle: 'H5:J5',
            subText: [
              ['Original', 'Modified', 'Avail Balance']
            ]
          },
          {
            fullRange: 'K4:N5',
        ...
        var titles = _.flattenDeep(_.map(ranges, function(val) {
          return val.titleText
        }));
        var subTitles = _.flattenDeep(_.map(ranges, function(val) {
          return val.subText
        }));
