  ...
  function($http, $scope, $rootScope, $state, $stateParams, budgetService, modalService, BookService) {
    ...
    $scope.selectedParent = function(parent) {
      ...
      if ('ferc' in parent) {
        ...
        else _.remove($scope.selectedKeys, {
          id: parent.id
        });
      }
    ...
    $scope.getSheetData = function() {
      ...
      if (keys && keys.length > 0) {
        ...
        if (runReport) {
          ...
          budgetService.getSheetData($scope.selectedValues.report.type, keys, $scope.selectedValues.month.name, 'Comp', $scope.selectedValues.dates.jdeYear, accounts, {
